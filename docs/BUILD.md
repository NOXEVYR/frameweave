...
将生成的 EXE 与 README、MIGRATION、LICENSE、THIRD_PARTY、PYTHON-LICENSE.txt 和 docs 放在同一个 `PrismCanvas-0.5.0/` 根目录下打包。源码包只包含明确允许的项目文件，不包含 venv、缓存、测试数据、用户工作流包库、模型、配置、日志或输入媒体。MCP 已在主程序中，不需单独守护进程或额外 SDK。
